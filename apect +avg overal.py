...
OUTPUT_CSV         = os.path.join(BASE_PATH, "final_output1.csv")
...
sent_tokenizer  = AutoTokenizer.from_pretrained(SENT_MODEL_NAME, use_fast=True)
sent_model      = AutoModelForSequenceClassification.from_pretrained(SENT_MODEL_NAME).to(device).eval()
...
def predict_sentiment_batched(texts: List[str]) -> List[Tuple[str, float, int]]:
    ...
    for i in range(0, len(texts), BATCH_SIZE):
        ...
        for p in probs:
            star = int(p.argmax()) + 1
            conf = float(p.max())
            if star in (1, 2): lab = "Negative"
            elif star == 3:   lab = "Neutral"
            else:             lab = "Positive"
...
def main():
    ...
    for rec, (lab, conf, stars) in zip(aspect_records_meta, sent_preds):
        out = {
            ...
            "Stars": int(stars),
            "Sentiment_Source": "NLPTown(semantic, aspect-first)"
        }
